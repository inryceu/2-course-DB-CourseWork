...
export class GameService {
  ...
  async update(id: number, updateGameDto: UpdateGameDto) {
    return this.prisma.executeTransaction(async (tx) => {
      ...
      if (updateGameDto.title) {
        const conflictGame = await tx.games.findFirst({
          where: {
  ...
  async addTagToGame(gameId: number, tagId: number) {
    return this.prisma.executeTransaction(async (tx) => {
      ...
      const existingConnection = await tx.game_tag_connection.findUnique({
        where: {
          game_id_tag_id: {
            game_id: gameId,
            tag_id: tagId,
      ...
      await tx.game_tag_connection.create({
        data: {
          game_id: gameId,
          tag_id: tagId,
        },
  ...
  async removeTagFromGame(gameId: number, tagId: number) {
    return this.prisma.executeTransaction(async (tx) => {
      const connection = await tx.game_tag_connection.findUnique({
        where: {
          game_id_tag_id: {
            game_id: gameId,
            tag_id: tagId,
      ...
      await tx.game_tag_connection.delete({
        where: {
          game_id_tag_id: {
            game_id: gameId,
            tag_id: tagId,
  ...
  async addDeveloperToGame(gameId: number, devId: number) {
    return this.prisma.executeTransaction(async (tx) => {
      ...
      const existingConnection = await tx.game_dev_connection.findUnique({
        where: {
          game_id_dev_id: {
            game_id: gameId,
            dev_id: devId,
      ...
      await tx.game_dev_connection.create({
        data: {
          game_id: gameId,
          dev_id: devId,
        },
  ...
  async removeDeveloperFromGame(gameId: number, devId: number) {
    return this.prisma.executeTransaction(async (tx) => {
      const connection = await tx.game_dev_connection.findUnique({
        where: {
          game_id_dev_id: {
            game_id: gameId,
            dev_id: devId,
      ...
      await tx.game_dev_connection.delete({
        where: {
          game_id_dev_id: {
            game_id: gameId,
            dev_id: devId,
